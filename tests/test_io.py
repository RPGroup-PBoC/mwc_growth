import pytest
import numpy as np
import pandas as pd
import sys
sys.path.insert(0, '../')
import mwc.io
import imp
imp.reload(mwc.io)


a = mwc.io.scrape_frontmatter('./test_data', file='test_accept.md')
b = mwc.io.scrape_frontmatter('./test_data', file='test_accept.md',
                              as_dataframe=True)
scrape_frontmatter = mwc.io.scrape_frontmatter


def test_scrape_frontmatter():
    accept_dict = {'status': 'accept', 'reason': 'test reason'}
    reject_dict = {'status': 'reject', 'reason': 'test reason'}
    questionable_dict = {'status': 'questionable',
                         'reason': 'test reason'}
    dicts = {'test_accept.md': accept_dict, 'test_reject.md': reject_dict,
             'test_questionable.md': questionable_dict}

    def cmp_dict(truth, test_file, directory, as_dataframe):
        test = scrape_frontmatter(directory, file=test_file,
                                  as_dataframe=as_dataframe)
        if as_dataframe == True:
            assert(truth.equals(test))

        else:
            assert test.keys() == truth.keys()
            assert test.items() == test.items()

    for k, v in zip(dicts.keys(), dicts.values()):
        cmp_dict(v, k, './test_data', False)
        cmp_dict(v, k, './test_data/', False)
        df_v = pd.DataFrame(v, index={0})
        cmp_dict(df_v, k, './test_data', True)


df_v
pd.cmp
