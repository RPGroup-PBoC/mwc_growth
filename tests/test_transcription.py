...
def test_MWC():
    ...
    assert sat == trans.MWC(c, ka, ki, ep_ai).saturation()
    assert leak == trans.MWC(c, ka, ki, ep_ai).leakiness()
    ...
    assert (test_c_pact == trans.MWC(c_range, ka, ki, ep_ai,
                                     n).pact()).all()
    assert (test_ka_pact == trans.MWC(c, ka_range, ki, ep_ai,
                                      n).pact()).all()
    assert (test_ki_pact == trans.MWC(c, ka, ki_range, ep_ai,
                                      n).pact()).all()
    assert (test_ep_ai_pact == trans.MWC(c, ka, ki, ep_ai_range,
                                         n).pact()).all()
    assert (test_n_pact == trans.MWC(c, ka, ki, ep_ai,
                                     n_range).pact()).all()
    ...
    c, ka, ki, ep_ai, n = np.meshgrid(c_range, ka_range, ki_range,
                                      ep_ai_range, n_range)
    test_pact = pact(c, ka, ki, ep_ai, n)
    assert (test_pact == trans.MWC(c, ka, ki, ep_ai, n).pact()).all()
