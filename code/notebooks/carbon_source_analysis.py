...
for g, d in c_data.groupby(['date', 'run_no']):
    stats = stat_dfs[(stat_dfs['carbon']==carbon)& (stat_dfs['date']==g[0]) &
                    (stat_dfs['run_no']==g[1])]
    param = stats[stats['parameter']=='alpha']
...
for g, d in fc_data.groupby(['carbon', 'date', 'run_number']):
    ...
    _stats = stat_dfs[(stat_dfs['carbon']==g[0]) & (stat_dfs['date']==g[1]) & 
                   (stat_dfs['run_no']==g[2]) & (stat_dfs['parameter']=='alpha')]
    if len(_stats) > 0:
