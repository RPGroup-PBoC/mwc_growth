/**
* global fitKa/Ki Fit Model
* --------------------------------------------------------------------------
* This Stan model fits the log transform of the dissociation constants Ka and
* Ki for the active and inactive state of the repressor, respectively. This is
* performed for a collection of strains and operators.
*
...
data {
    int<lower=0> n_samp; //Total number of samples.
    vector[n_samp] fc; // Experimentally measured fold-change
    vector[n_samp] c; // Inducer concentration
    vector<lower=0>[n_samp] R; //Number of repressors per cell.
    real<lower=0> Nns;  // Number of nonspecific binding sites for repressor.
    real ep_ai; // Allosteric energy difference.
    vector[n_samp] ep_r;  // DNA binding energy of repressor.
...
model {
    ...
    vector[n_samp] fc_theo;
    for (i in 1:n_samp){
          fc_theo[i] = fold_change(R[i], Nns, ep_r[i], c[i],
