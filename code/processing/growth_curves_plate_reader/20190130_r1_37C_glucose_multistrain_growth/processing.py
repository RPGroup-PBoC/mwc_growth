...
strain_wells = {**{alphanumeric_map[f'{row}{col}']:'delta' for row in ['E'] for col in np.arange(3, 11, 1)},
                **{alphanumeric_map[f'{row}{col}']:'manuel_delta' for row in ['F'] for col in np.arange(3, 11, 1)},
                **{alphanumeric_map[f'{row}{col}']:'geneva_LE' for row in ['D'] for col in np.arange(3, 11, 1)},
                **{alphanumeric_map[f'{row}{col}']:'geneva_BW' for row in ['C'] for col in np.arange(3, 11, 1)}
               }
carbon_wells = {**{alphanumeric_map[f'{row}{col}']:'glucose' for row in ['C','D','E','F'] 
                   for col in np.arange(3, 11, 1)}}
