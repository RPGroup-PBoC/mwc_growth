...
strain_wells = {**{alphanumeric_map[f'{row}{col}']:'delta' for row in ['E','F','G'] for col in np.arange(2, 12, 1)},
                **{alphanumeric_map[f'{row}{col}']:'manuel_delta' for row in ['B'] for col in np.arange(2, 12, 1)},
                **{alphanumeric_map[f'{row}{col}']:'geneva_LE' for row in ['C'] for col in np.arange(2, 12, 1)},
                **{alphanumeric_map[f'{row}{col}']:'geneva_BW' for row in ['D'] for col in np.arange(2, 12, 1)}
               }
carbon_wells = {**{alphanumeric_map[f'{row}{col}']:'glucose' for row in ['B','C','D','E'] for col in np.arange(2, 12, 1)},
                **{alphanumeric_map[f'{row}{col}']:'glycerol' for row in ['F'] for col in np.arange(2, 12, 1)}, 
                **{alphanumeric_map[f'{row}{col}']:'acetate' for row in ['G'] for col in np.arange(2, 12, 1)}}
