...
strain_wells = {**{alphanumeric_map[f'{row}{col}']:'delta' 
                   for row in ['B','C','D','E','F','G'] for col in np.arange(3, 10+1, 1)}}
carbon_wells = {**{alphanumeric_map[f'{row}{col}']:'glucose' 
                   for row in ['B','D','E','G'] for col in np.arange(3, 10+1, 1)},
                **{alphanumeric_map[f'{row}{col}']:'apromycin' 
                   for row in ['C','F'] for col in np.arange(3, 10+1, 1)},
                **{alphanumeric_map[f'{row}{col}']:'rifampicin' 
                   for row in ['D','G'] for col in np.arange(3, 10+1, 1)}}
antibiotic_wells = {**{alphanumeric_map[f'{row}{col}']:conc 
                   for row in ['C','F'] for (col,conc) in zip(
                       np.arange(3, 10+1, 1), [1000, 1500, 2000, 2250, 2500, 2750, 3000, 3750]
                       )},
                    **{alphanumeric_map[f'{row}{col}']:conc 
                   for row in ['D','G'] for (col,conc) in zip(
                       np.arange(3, 10+1, 1), [2800, 3200, 3600, 4000, 6000, 8000, 10000, 12000]
                       )}}
...
# Include strain IDs, carbons, and antibiotic concentrations.
for well in data['well_id'].unique():
    ...
    if well in antibiotic_wells.keys():
        data.loc[data['well_id']==well, 'ab_conc'] = antibiotic_wells[well]
    else: data.loc[data['well_id']==well, 'ab_conc'] = 0
...
ax[2].set_title('Antibiotic Concentration', y=1.12, fontsize = 13)
for i in range(3):
...
for row in np.arange(0,8,1): 
    for column in np.arange(0,12,1):
        ax[2].scatter(column+1, 8-row, s = 150, 
                    marker='$'+str(data[data['well_id']==alphanumeric_map[f'{row_letters[row]}{column+1}']]['ab_conc'].values[0]/1000)+'$', 
                    c=carbon_color_dict[carbon_map[row][column]]
                    )
