...
strain_wells = {**{alphanumeric_map[f'{row}{col}']:'delta' 
                   for row in ['B','C','D','E','F','G'] for col in np.arange(2, 11+1, 1)}}
carbon_wells = {**{alphanumeric_map[f'{row}{col}']:'glucose' 
                   for row in ['B','D','E','G'] for col in np.arange(2, 11+1, 1)},
                **{alphanumeric_map[f'{row}{col}']:'streptomycin' 
                   for row in ['C'] for col in np.arange(2, 11+1, 1)},
                **{alphanumeric_map[f'{row}{col}']:'rifamycin' 
                   for row in ['F'] for col in np.arange(2, 11+1, 1)}}
antibiotic_wells = {**{alphanumeric_map[f'{row}{col}']:conc 
                   for row in ['F'] for (col,conc) in zip(
                       np.arange(2, 11+1, 1), [5,10,50,100,500,1000,10000,50000]
                       )}}
...
for well in data['well_id'].unique():
    ...
    if well in antibiotic_wells.keys():
        data.loc[data['well_id']==well, 'antibiotic'] = antibiotic_wells[well]
    else: data.loc[data['well_id']==well, 'antibiotic'] = 0
