...
alpha_hpd_min, alpha_hpd_max = mwc.stats.compute_hpd(samples_df['alpha'], 0.95)
...
for i, c in enumerate(concs):
    ...
    _df = mwc.process.morphological_filter(_df, ip_dist=IP_DIST, 
               area_bounds=[0, 50], ar_bounds=[0, 1])
    ...
    dfs.append(_df[['strain', 'area_pix', 'mean_yfp', 'mean_mCherry', 
                   ...
                   'fold_change', 'volume', 'long_axis_death', 'short_axis_death']])
fc_df = pd.concat(dfs)
