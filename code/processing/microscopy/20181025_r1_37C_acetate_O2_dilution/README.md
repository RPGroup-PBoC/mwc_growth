...
## Notes & Observations
* The `auto` and `deltaLacI` strains were at different OD's than the `dilution` strains. These samples were 
back diluted to OD's comparable to the dilution strains and allowed to grow until all cells were around OD_600nm_ ~ 0.15-0.30.
...
3. Tubes were placed in a rack and covered with a plastic box to protect from photocleavage of ATC. Cells were allowed to grow for ~ 23 hours at 37°C with shaking at ~ 220 RPM. ~ 2 hours before harvesting, the `auto` and `deltaLacI` strains were diluted 1:3 and 1:2 respectively to bring them to OD's comparable to the `dilution` strains.
...
4. The other samples (`autofluorescence`, `deltaLacI`, and `dilution` for all ATC concentrations) were diluted 1:10 into a growth medium with no available carbon. Aliquots of 1µL were added to 3% agarose pads made of the growth medium with no added carbon.
