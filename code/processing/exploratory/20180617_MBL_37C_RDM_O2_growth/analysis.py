...
model = pystan.StanModel(
    '../../stan/hierarchical_growth_curve_single_condition.stan')
...
t_double_mode = np.log(
    2) / stats[stats['parameter'] == 'lambda_mu']['mode'].values[0]
t_double_max = np.log(2) / stats[stats['parameter']
                                 == 'lambda_mu']['hpd_min'].values[0]
t_double_min = np.log(2) / stats[stats['parameter']
                                 == 'lambda_mu']['hpd_max'].values[0]

# Compute the best fit and credible regions from the mode/HPD
time_range = np.linspace(0, data['time_min'].max() + 10, 500)
best_fit = np.exp(
    time_range * stats[stats['parameter'] == 'lambda_mu']['mode'].values[0])
upper = np.exp(
    time_range * stats[stats['parameter'] == 'lambda_mu']['hpd_max'].values[0])
lower = np.exp(
    time_range * stats[stats['parameter'] == 'lambda_mu']['hpd_min'].values[0])
...
_ = ax[0].fill_between(time_range, lower, upper,
                       color=colors[1], alpha=0.5, label='__nolegend__')

# Plot the data.
_ = ax[0].semilogy(data['time_min'], data['fractional_area'],
                   '.', ms=1, color=colors[0], label='microcolonies')
grouped = pd.DataFrame(data.groupby('time_min').apply(
    mwc.stats.compute_mean_sem, key='fractional_area')).reset_index()
...
plt.tight_layout()
plt.savefig('output/{}_{}_{}C_{}_{}_growth_curves.png'.format(DATE, RUN_NO, TEMPERATURE, CARBON, OPERATOR))
