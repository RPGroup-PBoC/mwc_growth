...
strain_wells = {**{alphanumeric_map[f'{row}{col}']:'delta' for row in ['B','C','D','E','F','G'] 
                   for col in np.arange(2, 12, 1)}}
carbon_wells = {**{alphanumeric_map[f'{row}{col}']:'glucose' for row in ['B','E'] for col in np.arange(2, 12, 1)}, 
                **{alphanumeric_map[f'{row}{col}']:'glycerol' for row in ['C','F'] for col in np.arange(2, 12, 1)},
                **{alphanumeric_map[f'{row}{col}']:'acetate' for row in ['D','G'] for col in np.arange(2, 12, 1)}}
